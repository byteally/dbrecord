...
insertMany _ rows = do
  let
    ...
    insertQ = InsertQuery (getTableId (Proxy @db) (Proxy @tab)) cnames (NE.fromList cexprss) Nothing []
  driver <- ask
  liftIO $ dbInsertRet driver insertQ
...
insertManyRet _ rows rets = do
  let
    ...
    cexprss = fmap toDBValues values
    values = fmap (\row -> toHList row (\v -> Identity v)) rows
    insertQ = InsertQuery (getTableId (Proxy @db) (Proxy @tab)) cnames (NE.fromList cexprss) Nothing (toPrimExprs rets)
  driver <- ask
  liftIO $ dbInsertRet driver insertQ
...
insert_ _ row = do
  let
    ...
    cexprs = toDBValues values
    values = toHList row (\v -> Identity v)
    insertQ = InsertQuery (getTableId (Proxy @sc) (Proxy @tab)) cnames (NE.fromList [cexprs]) Nothing []
  driver <- ask
