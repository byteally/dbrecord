...
order :: OrdExpr sc a => PQ.OrderOp -> Expr sc scopes a -> Order sc scopes
order op (Expr expr) = Order $ [PQ.OrderExpr op expr]

asc :: OrdExpr sc a => Expr sc scopes a -> Order sc scopes
asc = order PQ.OrderOp { PQ.orderDirection = PQ.OpAsc
                       , PQ.orderNulls = PQ.NullsLast
                       }

desc :: OrdExpr sc a => Expr sc scopes a -> Order sc scopes
desc = order PQ.OrderOp { PQ.orderDirection = PQ.OpDesc
                        , PQ.orderNulls = PQ.NullsFirst
                        }

ascNullsFirst :: OrdExpr sc a => Expr sc scopes a -> Order sc scopes
ascNullsFirst = order PQ.OrderOp { PQ.orderDirection = PQ.OpAsc
                                 , PQ.orderNulls = PQ.NullsFirst
                                 }

descNullsLast :: OrdExpr sc a => Expr sc scopes a -> Order sc scopes
