...
import DBRecord.Internal.Query (Nest (..))
...
testQ1 :: IO [Category]
testQ1 = runReaderT 'a' $ runPostgresDB $ runQueryAsList $ rel @(DVDRentalDB 'Postgres) @Category $ selectAll

hprop_additionCommutative :: Property
hprop_additionCommutative = property $ do
  xs <- forAll $ Gen.list (Range.linear 0 100) Gen.alpha
  reverse (reverse xs) === (head xs: xs)
